const SUPABASE_URL = "https://jwfpdzsruxaytnhuflhz.supabase.co"; // e.g. https://xxxxxxxx.supabase.co
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imp3ZnBkenNydXhheXRuaHVmbGh6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODg0NTQxMzIsImV4cCI6MjEwNDAzMDEzMn0.LlooBiaUUR1DbLprB7THoZBOddx2EUtm4Tuey5qoYD4"; // Project Settings > API > anon public key
const supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);

let contacts=[], currentView="all";

function openDB(){
 return supabase.from("contacts").select("id").limit(1).then(({error})=>{if(error) throw error});
}
function all(){
 return supabase.from("contacts").select("*").order("number",{ascending:true}).then(({data,error})=>{if(error) throw error; return data});
}
function put(c){
 return supabase.from("contacts").insert(c).select().single().then(({data,error})=>{if(error) throw error; return data.id});
}
function update(c){
 return supabase.from("contacts").update(c).eq("id",c.id).then(({error})=>{if(error) throw error});
}
function del(id){
 return supabase.from("contacts").delete().eq("id",id).then(({error})=>{if(error) throw error});
}
function clearDB(){
 return supabase.from("contacts").delete().neq("id",0).then(({error})=>{if(error) throw error});
}
...
async function add(b,a,p){
 ...
 try{
  ...
  if(err.code==="23505"){alert(`DUPLICATE NUMBER DETECTED: ${p}`);}
