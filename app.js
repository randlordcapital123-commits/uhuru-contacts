...
function render(){
 ...
 const list=contacts.filter(c=>(`${c.business||""} ${c.address||""} ${c.phone||""} ${c.number||""}`).toLowerCase().includes(q));
 document.querySelector("#total").textContent=contacts.length;
 document.querySelector("#contacts").innerHTML=list.map(c=>`
 <div class="row">
  ...
  <div class="call-info">
   <span class="called-badge ${c.called?"called":"not-called"}">${c.called?"✓ CALLED":"NOT CALLED"}</span>
   <small>${c.called?esc(formatDate(c.lastCalled)):"No call recorded"}</small>
   ${c.notes?`<div class="note-preview">📝 ${esc(c.notes)}</div>`:""}
  </div>
  <div class="actions">
   <button class="whatsapp" onclick="openWhatsApp(${c.id})">WHATSAPP</button>
   <button class="called" onclick="markCalled(${c.id})">${c.called?"CALLED ✓":"I CALLED"}</button>
   <button onclick="editNotes(${c.id})">NOTES</button>
   <button onclick="editContact(${c.id})">EDIT</button>
   <button class="danger" onclick="removeContact(${c.id})">DEL</button>
  </div>
 </div>`).join("");
...
async function add(b,a,p){
 ...
 await put({number:nextNumber(),business:b,address:a,phone:p,created:new Date().toISOString(),called:false,lastCalled:null,notes:""});
